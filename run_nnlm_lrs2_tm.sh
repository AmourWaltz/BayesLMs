...
if [ "$uncertainty" == "Bayesian" ]; then
    pytorch_path=lrs2/pytorch-${model_type}-emb${embedding_dim}_hid${hidden_dim}_nly${nlayers}-${dropout}-${uncertainty}-${T_bayes_pos}-pre${prior}-${mark}
    nn_model=$pytorch_path/model.pt
    decode_dir_suffix=pytorch-${model_type}-emb${embedding_dim}_hid${hidden_dim}_nly${nlayers}-${dropout}-${uncertainty}-${T_bayes_pos}-pre${prior}-${mark}-itpr${itpr}-ib${inter_flag}-${inter_alpha}
elif [ "$uncertainty" == "Gaussian" ]; then
    pytorch_path=lrs2/pytorch-${model_type}-emb${embedding_dim}_hid${hidden_dim}_nly${nlayers}-${dropout}-${uncertainty}-GP${T_gauss_pos}-pre${prior}-${mark}
    nn_model=$pytorch_path/model.pt
    decode_dir_suffix=pytorch-${model_type}-emb${embedding_dim}_hid${hidden_dim}_nly${nlayers}-${dropout}-${uncertainty}-GP${T_gauss_pos}-pre${prior}-${mark}-itpr${itpr}-ib${inter_flag}-${inter_alpha}
else
    pytorch_path=lrs2/pytorch-${model_type}-emb${embedding_dim}_hid${hidden_dim}_nly${nlayers}-${dropout}-${uncertainty}-GP${T_gauss_pos}-pre${prior}-${mark}
    nn_model=$pytorch_path/model.pt
...
if [ $stage -le 2 ]; then
  ...
  for decode_set in clean_avsr TF MVDR FAS; do
      ...
      steps/pytorchnn/lmrescore_nbest_pytorchnn_jwyu.sh \
        ...
        ${decode_dir}_${decode_dir_suffix}_jwyu
  done
fi
exit 0
