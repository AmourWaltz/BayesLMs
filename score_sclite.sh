...
case "$name" in
  eval2000*)
    ...
    ;;
  rt02*)
    # Score only the, swbd part 1...
    if [ $stage -le 3 ]; then
      for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
        $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.swbd1.LMWT.${wip}.log \
          grep -v '^sw_' $data/stm '>' $dir/score_LMWT_${wip}/stm.swbd1 '&&' \
          grep -v '^sw_' $dir/score_LMWT_${wip}/${name}.ctm '>' $dir/score_LMWT_${wip}/${name}.ctm.swbd1 '&&' \
          $hubscr -p $hubdir -V -l english -h hub5 -g $data/glm -r $dir/score_LMWT_${wip}/stm.swbd1 $dir/score_LMWT_${wip}/${name}.ctm.swbd1 || exit 1;
      done
    fi   
    # Score only the, swbd part 2...
    if [ $stage -le 3 ]; then
      for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
        $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.swbd2.LMWT.${wip}.log \
          grep -v -E '^sw_4\|^sw4' $data/stm '>' $dir/score_LMWT_${wip}/stm.swbd2 '&&' \
          grep -v -E '^sw_4\|^sw4' $dir/score_LMWT_${wip}/${name}.ctm '>' $dir/score_LMWT_${wip}/${name}.ctm.swbd2 '&&' \
          $hubscr -p $hubdir -V -l english -h hub5 -g $data/glm -r $dir/score_LMWT_${wip}/stm.swbd2 $dir/score_LMWT_${wip}/${name}.ctm.swbd2 || exit 1;
      done
    fi    
    # Score only the, swbd part 3...
    if [ $stage -le 3 ]; then
      for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
        $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.swbd3.LMWT.${wip}.log \
          grep -v -E '^sw_3\|^sw4' $data/stm '>' $dir/score_LMWT_${wip}/stm.swbd3 '&&' \
          grep -v -E '^sw_3\|^sw4' $dir/score_LMWT_${wip}/${name}.ctm '>' $dir/score_LMWT_${wip}/${name}.ctm.swbd3 '&&' \
          $hubscr -p $hubdir -V -l english -h hub5 -g $data/glm -r $dir/score_LMWT_${wip}/stm.swbd3 $dir/score_LMWT_${wip}/${name}.ctm.swbd3 || exit 1;
      done
    fi
    ;; 
  rt03* )
