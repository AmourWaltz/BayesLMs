        ...
        if args.uncertainty == 'Bayesian':
            if args.model == 'LSTM':
                ...
                if args.T_bayes_pos == 'FFN':
                    ...
                    #kl_loss += model.transformerlayers[1].self_attn.o_net.kl_divergence() / len(train_data) * args.seq_len
                    #kl_loss += model.transformerlayers[2].self_attn.o_net.kl_divergence() / len(train_data) * args.seq_len
                    #kl_loss += model.transformerlayers[3].self_attn.o_net.kl_divergence() / len(train_data) * args.seq_len
                    #kl_loss += model.transformerlayers[4].self_attn.o_net.kl_divergence() / len(train_data) * args.seq_len
                    #kl_loss += model.transformerlayers[5].self_attn.o_net.kl_divergence() / len(train_data) * args.seq_len
                    pass
...
#            print("forward time: ", forward_total)
#            print("backward time: ", backward_total)
#            forward_total = 0.
#            backward_total = 0.
            total_loss = 0.
            start_time = time.time()
...
#            print(model_dict['transformerlayers.5.gpnn.coef_mean'].mean(dim=1))
            ...
            if len(args.L_gauss_pos) < 3:
                ...
                print(model_dict['rnn.rnn.1.gpnn.coef_mean'].mean(dim=1))
            else:
                print(model_dict['rnn.rnn.0.gpnn.coef_mean'].mean(dim=1))
                print(model_dict['rnn.rnn.1.gpnn.coef_mean'].mean(dim=1))
